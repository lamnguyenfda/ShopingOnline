...
public class ForgotController {
	...
	public String send(@ModelAttribute("forgotPassword") @Validated ForgotPassword entity, BindingResult result,
			Model model) {
		model.addAttribute("check", false);
		if (!result.hasErrors()) {
			User user = dao.findByEmail(entity.getEmail());
			service.queue(entity.getEmail(), "Làm mới mật khẩu!",
					"Vui lòng click vào link này: " + "http://localhost:8080/reset-password/?code=" + user.getPassword()
